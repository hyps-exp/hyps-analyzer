...
void G4Simulation::Loop(){
  ...
  fChain->SetBranchStatus("vertexLambdaNConv",1);//3
  fChain->SetBranchStatus("vertexLambdaDecay",1);//3
  ...
  TH1D *h0[NumOfMomHist][NbinCos], *h1[NumOfMomHist][NbinCos], *h2[NumOfMomHist][NbinCos], *h3[NumOfMomHist][NbinCos], *h4[NumOfMomHist][NbinCos],
	*h5[NumOfMomHist][NbinCos], *h6[NumOfMomHist][NbinCos], *h7[NumOfMomHist][NbinCos], *h8[NumOfMomHist][NbinCos];
  TH1D *hh0[NumOfMomHist], *hh1[NumOfMomHist], *hh2[NumOfMomHist], *hh3[NumOfMomHist], *hh4[NumOfMomHist], *hh5[NumOfMomHist],
	*hh6[NumOfMomHist], *hh7[NumOfMomHist], *hh8[NumOfMomHist], *hh9[NumOfMomHist], *hh10[NumOfMomHist];

  char buf[100], buf2[100];
  for(int i=0; i<NumOfMomHist; i++){
	for(int j=0; j<NbinCos; j++ ){
	  sprintf(buf, "h0_%d_%d", i,j);
	  sprintf(buf2, "dp (All) (%.2lf < p < %.2lf)", MinMom[i], MaxMom[i]);
	  h0[i][j] = new TH1D(buf, buf2, Nbindp, -0.5, 0.5);
	  
	  sprintf(buf, "h1_%d_%d", i,j);
	  sprintf(buf2, "dp (Detected requirement) (%.2lf < p < %.2lf)", MinMom[i], MaxMom[i]);
	  h1[i][j] = new TH1D(buf, buf2, Nbindp, -0.5, 0.5);
	  
	  sprintf(buf, "h2_%d_%d", i,j);
	  sprintf(buf2, "dp (Detected && cdist1 requirement) (%.2lf < p < %.2lf)", MinMom[i], MaxMom[i]);
	  h2[i][j] = new TH1D(buf, buf2, Nbindp, -0.5, 0.5);
	  
	  sprintf(buf, "h3_%d_%d", i,j);
	  sprintf(buf2, "dp (Detected && dz1 requirement)(%.2lf < p < %.2lf)", MinMom[i], MaxMom[i]);
	  h3[i][j] = new TH1D(buf, buf2, Nbindp, -0.5, 0.5);
	  
	  sprintf(buf, "h4_%d_%d", i,j);
	  sprintf(buf2, "dp (Detected && cdist2 requirement) (%.2lf < p < %.2lf)", MinMom[i], MaxMom[i]);
	  h4[i][j] = new TH1D(buf, buf2, Nbindp, -0.5, 0.5);

	  sprintf(buf, "h5_%d_%d", i,j);
	  sprintf(buf2, "dp (Detected && dz2 requirement) (%.2lf < p < %.2lf)", MinMom[i], MaxMom[i]);
	  h5[i][j] = new TH1D(buf, buf2, Nbindp, -0.5, 0.5);

	  sprintf(buf, "h6_%d_%d", i,j);
	  sprintf(buf2, "dp (Detected && NPCut requirement) (%.2lf < p < %.2lf)", MinMom[i], MaxMom[i]);
	  h6[i][j] = new TH1D(buf, buf2, Nbindp, -0.5, 0.5);

	  sprintf(buf, "h7_%d_%d", i,j);
	  sprintf(buf2, "dp (Detected && LambdaNCut requirement) (%.2lf < p < %.2lf)", MinMom[i], MaxMom[i]);
	  h7[i][j] = new TH1D(buf, buf2, Nbindp, -0.5, 0.5);

	  sprintf(buf, "h8_%d_%d", i,j);
	  sprintf(buf2, "dp (Detected && AllCut requirement) (%.2lf < p < %.2lf)", MinMom[i], MaxMom[i]);
	  h8[i][j] = new TH1D(buf, buf2, Nbindp, -0.5, 0.5);
	}
	...
	sprintf(buf2, "costCM (Detected && SigmaPCut requirement) (%.2lf < p < %.2lf)", MinMom[i], MaxMom[i]);
  ...
  for(int evenum =0; evenum<nev; ++evenum ){
	...
	for( int i=0; i<3; i++ ){
	  vertexLambdaNConv[i]=-9999;
	  vertexLambdaDecay[i]=-9999;
	}
	scatFlag=-1;
	thetaScatHypCM=-9999;
	cdistScat=-9999;
	...
	if( cdistLambdaNConv<Limit_Sim_cdistLambdaNConv ) cdist1_flag=true;
	bool dz1_flag=false;
	if( vertexLambdaNConv[2]-Vertex_z[0]>Limit_Sim_dz1 ) dz1_flag=true;
	bool cdist2_flag=false;
	if( cdistLambdaDecay<Limit_Sim_cdistLambdaDecay ) cdist2_flag=true;
	bool dz2_flag=false;
	if( vertexLambdaDecay[2]-vertexLambdaNConv[2]>Limit_Sim_dz2 ) dz2_flag=true;
	bool NPCut_flag=false;
	if( abs(EkinCorP-scatNpEkinCal)>Limit_Sim_NPCut ) NPCut_flag=true;
	bool SigmaPCut_flag=false;
	if( abs(EkinCorP-scatEkinCal)>Limit_Sim_SigmaPCut ) SigmaPCut_flag=true;
	...
	h0[index][cosbin-1]->Fill( dp );
	hh0[index]->Fill( cos(thetaCM*Deg2Rad) );
	if( target_flag && nPi_CFT>0 && nP_CFT>0 && dp>Limit_Sim_Detectdp ){//Detected
	  h1[index][cosbin-1]->Fill( dp );
	  hh1[index]->Fill( cos(thetaCM*Deg2Rad) );
	  if( cdist1_flag ){
	  h2[index][cosbin-1]->Fill( dp );
	  hh2[index]->Fill( cos(thetaCM*Deg2Rad) );		
	  }
	  if( dz1_flag ){
		h3[index][cosbin-1]->Fill( dp );
		hh3[index]->Fill( cos(thetaCM*Deg2Rad) );
	  }
	  if( cdist2_flag ){
		h4[index][cosbin-1]->Fill( dp );
		hh4[index]->Fill( cos(thetaCM*Deg2Rad) );	
   	  }
	  if( dz2_flag ){
		h5[index][cosbin-1]->Fill( dp );
		hh5[index]->Fill( cos(thetaCM*Deg2Rad) );
	  }
	  if( NPCut_flag ){
		h6[index][cosbin-1]->Fill( dp );
		hh6[index]->Fill( cos(thetaCM*Deg2Rad) );
	  }
	  if( SigmaPCut_flag ){
		h7[index][cosbin-1]->Fill( dp );
		hh7[index]->Fill( cos(thetaCM*Deg2Rad) );
	  }
	  if( cdist1_flag && dz1_flag && cdist2_flag && dz2_flag && NPCut_flag /*&& SigmaPCut*/ ){
		h8[index][cosbin-1]->Fill( dp );
  ...
  for(int i=0; i<NumOfMomHist; i++){
	for (int j=1; j<=NbinCos; j++){
	  double val1 = h8[i][j]->Integral(h8[i][j]->FindBin(-0.5),h8[i][j]->FindBin(0.5));
	  double val2 = h1[i][j]->Integral(h1[i][j]->FindBin(-0.5),h1[i][j]->FindBin(0.5));
	  if(val2 > 0){
		hh9[i]->SetBinContent(j, val1/val2);
		hh9[i]->SetBinError(j, 1./val2*sqrt(val1*(val2-val1)/val2));
	  }

	  val1 = h8[i][j]->Integral(h8[i][j]->FindBin(-Limit_Sim_Cutdp),h8[i][j]->FindBin(Limit_Sim_Cutdp));
	  if(val2 > 0){
  ...
  FILE *fp = fopen("LambdaNConvCutEfficiency.txt","w");
  ...
  for( int i=0; i<2; i++ ){
	...
	for( int j=0; j<NumOfMomHist; j++ ){
	  ...
	  for( int k=1; k<=NbinCos; k++ ){
		if( i==0 ) fprintf( fp, " %.3lf", hh9[j]->GetBinContent(k) );
		if( i==1 ) fprintf( fp, " %.3lf", hh10[j]->GetBinContent(k) );
  ...
  for( int i=0; i<2; i++ ){
	...
	for( int j=0; j<NumOfMomHist; j++ ){
	  fprintf(fp, "{ ");
	  for( int k=1; k<=NbinCos; k++ ){
		if( i==0 ) fprintf( fp, " %.3lf", hh9[j]->GetBinError(k) );
		if( i==1 ) fprintf( fp, " %.3lf", hh10[j]->GetBinError(k) );
		if( k!=NbinCos ) fprintf(fp, ", ");
	  }
	  fprintf(fp, "}");
	  if( j!=NumOfMomHist-1 ) fprintf(fp, ", \n");
  ...
  ///write rootfile////
  TFile *fout = new TFile("ForLambdaNConvCutEfficiency.root", "recreate");
  for( int i=0; i<NumOfMomHist; i++ ){
	for( int j=0; j<NbinCos; j++ ){
	  h0[i][j]->Write();
	  h1[i][j]->Write();
...
//////////////////////////////////////////////////////////////////////////
double GetFlightLength( TVector3 v1, TVector3 v2, TVector3 v3 ){
  TVector3 VertexLength1 = v1+v2;
  TVector3 VertexLength2 = v2+v3;
