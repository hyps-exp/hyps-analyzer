...
ProcessingNormal()
{
  ...
  std::vector<Int_t> optlink_fe_id;
  ...
  for(auto&& c : gUnpacker.get_root()->get_child_list()) {
    ...
    if (n.Contains("optlink"))
      optlink_fe_id.push_back(id);
    if (n.Contains("hul"))
...
ConfMan::InitializeHistograms()
{
  ...
  daq->Branch("eb_datasize",     &event.eb_datasize,     "eb_datasize/I");
  daq->Branch("vme_datasize",     &event.vme_datasize,     "vme_datasize/I");
  daq->Branch("optlink_datasize",     &event.optlink_datasize,     "optlink_datasize/I");
  daq->Branch("hul_datasize",     &event.hul_datasize,     "hul_datasize/I");
  daq->Branch("veaas0c_datasize",     &event.vea0c_datasize,     "vea0c_datasize/I");
