...
FieldMan::~FieldMan()
{
  if(m_hyps_map) delete m_hyps_map;
...
FieldMan::Initialize()
{
  ...
  if(!m_file_name_hyps.IsNull()){
    m_hyps_map = new HypsFieldMap(m_file_name_hyps, valueNMR, valueCalc);
    if(!m_hyps_map->Initialize())
      return false;
  }
  ...
  if(!m_file_name_shs.IsNull()){
    std::cout << m_file_name_shs << " " << m_file_name_shs.IsNull() << std::endl;
    m_shs_map = new HypsFieldMap(m_file_name_shs, valueHSHall, valueHSCalc);
    if(!m_shs_map->Initialize())
      return false;
  }

  m_is_ready = (m_hyps_map || m_shs_map);
  return m_is_ready;
}

//_____________________________________________________________________________
Bool_t
FieldMan::Initialize(const TString& file_name_hyps)
{
  m_file_name_hyps = file_name_hyps;
  return Initialize();
}
...
FieldMan::GetField(const TVector3& position) const
{
  ...
  if(m_hyps_map // && m_shs_map
    ){
    Double_t p[3], b_hyps[3]; //, b_shs[3];
    p[0] = position.x()*0.1;
    p[1] = position.y()*0.1;
    p[2] = position.z()*0.1;
    if(m_hyps_map->GetFieldValue(p, b_hyps)//  &&
